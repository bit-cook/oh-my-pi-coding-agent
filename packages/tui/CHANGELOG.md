...
- Fixed a terminal resize during foreground-tool streaming on an unknown-viewport / ED3-risk host (Ghostty/kitty/Alacritty/iTerm2/WSL) leaving native scrollback permanently out of sync, so scrolling back after the turn showed missing rows. A pure geometry resize (no content change) takes the in-place viewport-repaint path, which — unlike a content-bearing resize that rebuilds via the geometry branch — never flagged native history. Because the prompt-submit checkpoint (`refreshNativeScrollbackIfDirty`) only rebuilds when scrollback is marked dirty on these hosts, the discrepancy was never reconciled. Overflowing geometry repaints whose viewport is not known to be at the bottom now mark scrollback dirty so the next checkpoint rebuilds an exact copy of the tran…
- Fixed `Editor` content rows leaking box-drawing characters (`│`, `╰─`, `─╯`) into terminal mouse-drag copy. Side rails were the leading and trailing non-whitespace cells on every content row; terminals can only trim trailing whitespace, so `│  text   │` and `╰─ text ─╯` showed up in pasted output verbatim. The content rows now render as `padding + text + trailing whitespace`, leaving only the top border (rarely selected, anchors the editor visually and carries `setTopBorder()` status content + the bash/python/thinking border color tint). ([#1812](https://github.com/can1357/oh-my-pi/issues/1812))
